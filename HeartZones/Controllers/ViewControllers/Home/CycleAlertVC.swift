...
import UIKit
protocol CycleVcDelegate {
    func CycleTapped(strType:String)
}

class CycleAlertVC: UIViewController {
     var CycleVcDelegate: CycleVcDelegate?
    ...
    @IBAction func ResumeAction(_ sender: Any) {
        CycleVcDelegate?.CycleTapped(strType: "Resume")
        self.dismiss(animated: true, completion: nil)
    }
    
    @IBAction func SaveAction(_ sender: Any) {
        CycleVcDelegate?.CycleTapped(strType: "Save")
        self.dismiss(animated: true, completion: nil)
    }
    
    @IBAction func Recovery1Action(_ sender: Any) {
        CycleVcDelegate?.CycleTapped(strType: "Recovery1")
        self.dismiss(animated: true, completion: nil)
    }
    
    @IBAction func Recovery2Action(_ sender: Any) {
        CycleVcDelegate?.CycleTapped(strType: "Recover2")
        self.dismiss(animated: true, completion: nil)
    }
    
    @IBAction func DiscardAction(_ sender: Any) {
        CycleVcDelegate?.CycleTapped(strType: "Discard")
        self.dismiss(animated: true, completion: nil)
    }
