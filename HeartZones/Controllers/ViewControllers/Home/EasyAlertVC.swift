...
import UIKit
protocol EasyVcDelegate {
    func EasyTapped(strType:String)
}
class EasyAlertVC: UIViewController {
     var EasyVcDelegate: EasyVcDelegate?
    ...
    @IBAction func ResumeAction(_ sender: Any) {
        EasyVcDelegate?.EasyTapped(strType: "Resume")
        self.dismiss(animated: true, completion: nil)
    }
    
    @IBAction func SaveAction(_ sender: Any) {
        EasyVcDelegate?.EasyTapped(strType: "Save")
        self.dismiss(animated: true, completion: nil)
    }
    
    @IBAction func Recovery1Action(_ sender: Any) {
        EasyVcDelegate?.EasyTapped(strType: "Recovery1")
        self.dismiss(animated: true, completion: nil)
    }
    
    @IBAction func Recovery2Action(_ sender: Any) {
        EasyVcDelegate?.EasyTapped(strType: "Recover2")
        self.dismiss(animated: true, completion: nil)
    }
    
    @IBAction func DiscardAction(_ sender: Any) {
         EasyVcDelegate?.EasyTapped(strType: "Discard")
        self.dismiss(animated: true, completion: nil)
    }
