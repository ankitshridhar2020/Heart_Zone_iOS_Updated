//
//  HomeDetailsModerateVC.swift
//  HeartZones
//
//  Created by Hitendra Lariya on 15/10/19.
...
class HomeDetailsBandsCVC : UICollectionViewCell {
    ...
    override func awakeFromNib() {
        ...
        case .phone:
            aaChartView.frame = CGRect(x: viewLineChart.frame.origin.x,
                                       y: 0,
                                       width: viewLineChart.frame.size.width + 40 ,
                                       height: viewLineChart.frame.size.height)
            break
        // It's an iPhone
        case .pad:
            ...
            aaChartView.frame = CGRect(x: viewLineChart.frame.origin.x,
                                       y: 0,
                                       width: viewLineChart.frame.size.width ,
                                       height: viewLineChart.frame.size.height)
            break
     ...
     // aaChartView.contentHeight = viewLineChart.frame.size.height - 80
        viewLineChart.addSubview(aaChartView)
        viewLineChart.contentMode = .scaleAspectFit
...
class HomeDetailsModerateVC: UIViewController, ModerateVcDelegate {
    ...
    @objc func PouseAction() {
        ...
        self.performSegue(withIdentifier: "GotoModerateAlert", sender: self)
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if (segue.identifier == "GotoModerateAlert") {
            let vc = segue.destination as! ModerateAlertVC
            vc.ModerateVcDelegate = self
        }
    }
    
   
}
extension HomeDetailsModerateVC: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if(indexPath.row == 0){
            ...
            let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "HomeDetailsCVC1", for: indexPath) as!  HomeDetailsCVC
            return cell
        }else  {
            let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "HomeDetailsCVC2", for: indexPath) as!  HomeDetailsCVC
            cell.contentView.bringSubviewToFront(cell.btnPouse)
            cell.btnPouse.addTarget(self, action: #selector(PouseAction), for: .touchUpInside)
...
extension HomeDetailsModerateVC: UICollectionViewDelegateFlowLayout {
...
extension HomeDetailsModerateVC:UIScrollViewDelegate{
